...
//BGM読み込み
	const bgmUrl = "hatimi.wav";  						//game.htmlからの相対パス
	...
	game.onload = function () {					//ロードが終わった後にこの関数が呼び出されるので、この関数内にゲームのプログラムを書こう
		...
		scoreText.color = 'rgba(150,150,150,1)';		//色　RGB+透明度　今回は白
		...
		game.onenterframe = function () {
			...
			if (state == 3) {							//状態３（point６以上から）
				zoyamaImg.x += 15;
				zoyamaImg.y = 100 + Math.sin(zoyamaImg.x / 70) * 100; // ｙ座標を振幅100pxのサイン波で移動(Sinは便利なので慣れとくといいよ！)
			}
			...
			if (zoyamaImg.x >= 400) {						//画面端にぞう山画像が行ってしまったら
				...
				//ゲームオーバー後のテキスト表示
				gameOverText.text = "テイオーステップ：" + point + "回達成！";				//テキストに文字表示 
			}
		...
		retryBtn.moveTo(150, 250);						//リトライボタンの位置
		...
		retryBtn.ontouchend = function () {				//S_Retryボタンをタッチした（タッチして離した）時にこの中の内容を実行する
			...
			game.assets[bgmUrl].clone().play();		//bgm開始
		};

		//ツイートボタン
		const tweetBtn = new Sprite(120, 60);				//画像サイズをここに書く。使う予定の画像サイズはプロパティで見ておくこと
		tweetBtn.moveTo(150, 330);						//リトライボタンの位置
